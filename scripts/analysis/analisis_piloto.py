...
raw_signal.filter(l_freq=5.0, h_freq=40, picks='emg', fir_design='firwin')
...
# reflejen solo el intervalo de interes (tmin_crop_plots..tmax_crop_plots).
psd = epochs.copy().crop(tmin_crop_plots, tmax_crop_plots).compute_psd(method='welch', fmin=4.0, fmax=30.0)
...
if show_figs:
    fig_psd_ind.show()
if save_figs:
    fig_psd_ind.savefig(os.path.join(fig_dir, f"{fig_prefix}_psd_ind.png"), dpi=300, bbox_inches='tight')
...
if show_figs:
    fig_psd_avg.show()
if save_figs:
    fig_psd_avg.savefig(os.path.join(fig_dir, f"{fig_prefix}_psd_avg.png"), dpi=300, bbox_inches='tight')
...
if show_figs:
    fig_topo_psd.show()
if save_figs:
    fig_topo_psd.savefig(os.path.join(fig_dir, f"{fig_prefix}_psd_topo.png"), dpi=300, bbox_inches='tight')
...
# Recortar al rango de visualizacion. La TFR se calcula sobre el epoch completo
# (tmin_epocs..tmax_epocs) para evitar artefactos de borde en los wavelets;
# el crop aqui solo afecta a los plots.
power = power.crop(tmin_crop_plots, tmax_crop_plots)
itc   = itc.crop(tmin_crop_plots, tmax_crop_plots)
...
fig_power_topo = power.plot_topo(
    baseline=baseline_topomaps, mode="logratio",
...
if show_figs:
    fig_power_topo.show()
if save_figs:
    fig_power_topo.savefig(os.path.join(fig_dir, f"{fig_prefix}_tfr_potencia_topo.png"), dpi=300, bbox_inches='tight')
...
if show_figs:
    fig_power_ch.show()
if save_figs:
    fig_power_ch.savefig(os.path.join(fig_dir, f"{fig_prefix}_tfr_potencia_ch-{power.ch_names[ch_idx]}.png"), dpi=300, bbox_inches='tight')
...
topomap_kw = dict(
    ch_type="eeg",
    tmin=tmin_crop_plots,
    tmax=tmax_crop_plots,
    baseline=baseline_topomaps,
    mode="logratio",
    show=False,
)
...
if save_figs:
    fig_power_bands.savefig(os.path.join(fig_dir, f"{fig_prefix}_tfr_potencia_bandas.png"), dpi=300, bbox_inches='tight')
...
if save_figs:
    fig_power_joint.savefig(os.path.join(fig_dir, f"{fig_prefix}_tfr_potencia_joint.png"), dpi=300, bbox_inches='tight')
...
if show_figs:
    fig_itc_topo.show()
if save_figs:
    fig_itc_topo.savefig(os.path.join(fig_dir, f"{fig_prefix}_tfr_itc_topo.png"), dpi=300, bbox_inches='tight')
...
if show_figs:
    fig_itc_ch.show()
if save_figs:
    fig_itc_ch.savefig(os.path.join(fig_dir, f"{fig_prefix}_tfr_itc_ch-{itc.ch_names[ch_idx]}.png"), dpi=300, bbox_inches='tight')
